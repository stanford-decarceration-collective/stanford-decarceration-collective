...
transitions_data = pd.concat(
    [
        transitions_data,
        pd.read_csv(
            "recidiviz/calculator/modeling/population_projection/state_recidiviz/OK/OK_habitual_sentencing/OK_data/Habitual Transitions Data-Table 1.csv"
        ),
    ]
)
...
yearly_outflows_data = pd.read_csv(
    "recidiviz/calculator/modeling/population_projection/state_recidiviz/OK/OK_habitual_sentencing/OK_data/Habitual Outflows Data (Yearly)-Table 1.csv"
)
    ...
    temp_monthly_outflows_data = pd.DataFrame(
        {
            ...
            * 10,
            ...
            "crime_type": ["max_sentence_1_yr"] * 12
            + ["max_sentence_2_yr"] * 12
            + ["max_sentence_3_yr"] * 12
            + ["max_sentence_4_yr"] * 12
            + ["max_sentence_5_yr"] * 12
            + ["max_sentence_7_yr"] * 12
            + ["max_sentence_8_yr"] * 12
            + ["max_sentence_10_yr"] * 12
            + ["max_sentence_15_yr"] * 12
            + ["max_sentence_20_yr"] * 12,
            "total_population": [
                yearly_outflows_data.iloc[(year - reference_year) * 10, 4] / 12
                for month in range(12)
            ]
            + [
                yearly_outflows_data.iloc[(year - reference_year) * 10 + 1, 4] / 12
                for month in range(12)
            ]
            + [
                yearly_outflows_data.iloc[(year - reference_year) * 10 + 2, 4] / 12
                for month in range(12)
            ]
            + [
                yearly_outflows_data.iloc[(year - reference_year) * 10 + 3, 4] / 12
                for month in range(12)
            ]
            + [
                yearly_outflows_data.iloc[(year - reference_year) * 10 + 4, 4] / 12
                for month in range(12)
            ]
            + [
...
upload_spark_model_inputs(
    ...
    "recidiviz/calculator/modeling/population_projection/state_recidiviz/OK/OK_habitual_sentencing/OK_prison_model_inputs.yaml",
)
