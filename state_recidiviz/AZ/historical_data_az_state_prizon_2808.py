...
historical_data_2019 = pd.read_csv(
    "recidiviz/calculator/modeling/population_projection/state_recidiviz/AZ/AZ_data/HB_2808/2019.csv",
    sep=";",
    thousands=",",
)
historical_data_2018 = pd.read_csv(
    "recidiviz/calculator/modeling/population_projection/state_recidiviz/AZ/AZ_data/HB_2808/2018.csv",
    sep=";",
    thousands=",",
)
historical_data_2017 = pd.read_csv(
    "recidiviz/calculator/modeling/population_projection/state_recidiviz/AZ/AZ_data/HB_2808/2017.csv",
    sep=";",
    thousands=",",
)
...
population_by_crime = {
    crime: get_yearly_fields_for_crime("total", crime) for crime in crimes
}
population_data = pd.DataFrame({"year": [-3, -2, -1], **population_by_crime})

admissions_by_crime = {
    crime: get_yearly_fields_for_crime("admissions", crime) for crime in crimes
}
admissions_data = pd.DataFrame({"year": [-3, -2, -1], **admissions_by_crime})

releases_by_crime = {
    crime: get_yearly_fields_for_crime("releases", crime) for crime in crimes
}
releases_data = pd.DataFrame({"year": [-3, -2, -1], **releases_by_crime})
...
average_LOS = {
    crime: np.mean(population_data[crime]) / np.mean(admissions_data[crime])
    for crime in crimes
}
...
pd.concat([transitions_data, outflows_data, total_population_data]).to_csv(
    "recidiviz/calculator/modeling/population_projection/state_recidiviz/AZ/preprocessed_data_AZ_prison_2808.csv"
)
